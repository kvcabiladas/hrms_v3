    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4">
            ...
            <div>
                ...
                <h3 class="text-2xl font-bold text-gray-800">{{ $stats['present'] }} <span class="text-xs font-normal text-gray-400">this month</span></h3>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4">
            ...
            <div>
                ...
                <h3 class="text-2xl font-bold text-gray-800">{{ number_format($stats['hours'], 1) }} <span class="text-xs font-normal text-gray-400">hrs</span></h3>
            </div>
        </div>
    </div>

    <!-- 2. Clock In/Out Action Area -->
    <div class="bg-gradient-to-r from-gray-800 to-gray-900 rounded-xl shadow-lg p-8 mb-8 text-white flex flex-col md:flex-row justify-between items-center">
        ...
        <div>
            ...
            @if(!$todayRecord)
                ...
                <form action="{{ route('attendance.store') }}" method="POST">
                    @csrf
                    <button class="px-8 py-4 bg-green-500 hover:bg-green-600 text-white rounded-full font-bold text-lg shadow-lg transform transition hover:scale-105 flex items-center gap-3">
            ...
            @elseif(!$todayRecord->clock_out)
                ...
                <div class="text-center">
                    ...
                    <form action="{{ route('attendance.update', $todayRecord->id) }}" method="POST">
                        @csrf @method('PUT')
                        <button class="px-8 py-4 bg-red-500 hover:bg-red-600 text-white rounded-full font-bold text-lg shadow-lg transform transition hover:scale-105 flex items-center gap-3">
